...
public class c5_CreatingSequence {
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(sink -> {if (counter.get() > 5){
            sink.complete();
        ...
        Flux<Integer> createFlux = Flux.create(sink -> { for (int i = 0; i <= 5; i++) {
            sink.next(i);
            //todo: fix following code so it emits values from 0 to 5 and then completes
        }sink.complete();});
        ...
        Flux<Integer> pushFlux = Flux.push(sink -> {for (int i = 0; i <= 5; i++) {
            sink.next(i);}
            sink.complete();
            //todo: fix following code so it emits values from 0 to 5 and then completes
        });
