...
public class c4_LifecycleHooks extends LifecycleHooksBase {
    ...
    public void no_subscription_no_gains() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service().doOnSubscribe((a) -> hooksTriggered.add("subscribe"));
    ...
    public void atomic_counter() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service().doOnNext(item -> {System.out.println(item); counter.incrementAndGet();})
                //todo: change this line only
                ;
    ...
    public void one_to_rule_them_all() {
        ...
        Flux<Integer> flux = Flux.just(1, 2, 3).doOnEach(integerSignal -> signals.add(integerSignal.getType().name()))
                //todo: change this line only
                ;
